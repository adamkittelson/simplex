defmodule SimplexTest do
  ...
  test "gets access_key from IAM" do
    :meck.expect(HTTPotion, :get, fn
      "http://169.254.169.254/latest/meta-data/iam/security-credentials/", [timeout: 500] ->
        %HTTPotion.Response{
          body: "simpledb",
          headers: [
      ...
      "http://169.254.169.254/latest/meta-data/iam/security-credentials/simpledb",
      [timeout: 500] ->
        %HTTPotion.Response{
          body:
  ...
  test "an expired key is expiring" do
    one_week_ago =
      DateTime.utc_now() |> Timex.shift(weeks: -1) |> Timex.format!("{ISO:Extended:Z}")
  ...
  test "a key that expires 30 seconds from now is expiring" do
    thirty_seconds_from_now =
      DateTime.utc_now() |> Timex.shift(seconds: 30) |> Timex.format!("{ISO:Extended:Z}")
  ...
  test "a key that expires 1 hour from now is not expiring" do
    one_hour_from_now =
      DateTime.utc_now() |> Timex.shift(hours: 1) |> Timex.format!("{ISO:Extended:Z}")
