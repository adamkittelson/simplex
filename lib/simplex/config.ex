defmodule Simplex.Config do
  ...
  def needs_refresh?(:aws_access_key, config) do
    expiring?(config) or (!config[:aws_access_key] and !System.get_env("SIMPLEX_AWS_ACCESS_KEY"))
  end

  def needs_refresh?(:aws_secret_access_key, config) do
    expiring?(config) or (!config[:aws_secret_access_key] and !System.get_env("SIMPLEX_AWS_SECRET_ACCESS_KEY"))
  end
